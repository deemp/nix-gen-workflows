...
rec {
  ...
  convertUses =
    x:
    x
    // (lib.optionalAttrs (x ? uses && builtins.isAttrs x.uses) (
      let
        uses = builtins.head (builtins.attrValues x.uses);
      in
      {
  ...
  convertNull_s = lib.mapAttrs (
    _: value:
    if value == lib.values.null_ then
      null
    else if builtins.isList value then
      map convertNull_s value
  ...
  resolveWorkflows =
    ...
    lib.pipe config.workflows [
      (lib.mapAttrs (
        ...
        // (
          let
            actions = lib.recursiveUpdate config.actions (workflow.actions or { });
          in
          {
            inherit actions;
            jobs = lib.mapAttrs (
